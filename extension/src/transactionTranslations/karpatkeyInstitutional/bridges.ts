...
export function extractBridgedTokenAddress({ to, data }: MetaTransactionData) {
  const bridge = MAINNET_BRIDGES.find(
    (bridge) => to.toLowerCase() === bridge.address.toLowerCase()
  )
