...
describe.sequential('List Accounts', () => {
  ...
  describe('List', () => {
    ...
    dbIt('lists only accounts from the selected workspace', async () => {
      const user = await userFactory.create()
      const tenant = await tenantFactory.create(user)
  ...
  describe('Edit', () => {
    dbIt('is possible to edit an account', async () => {
      const user = await userFactory.create()
      const tenant = await tenantFactory.create(user)
  ...
  describe('Move', () => {
    dbIt(
      'is possible to move an account to a different workspace',
      async () => {
        const user = await userFactory.create()
        const tenant = await tenantFactory.create(user)
        const account = await accountFactory.create(tenant, user)
        ...
        ).resolves.toHaveProperty('workspaceId', workspace.id)
      },
    )
