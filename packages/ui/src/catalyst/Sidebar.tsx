...
export function Sidebar({
  ...
  return (
    <nav
      {...props}
      className={classNames(className, 'flex h-full min-h-0 flex-col')}
    />
  )
}
...
export function SidebarSpacer({
  ...
  return (
    <div
      aria-hidden="true"
      {...props}
      className={classNames(className, 'mt-8 flex-1')}
    />
  )
}
    ...
    <span className={classNames(className, 'relative')}>
      {current && (
        <motion.span
          ...
          className="absolute inset-y-2 -left-4 w-0.5 rounded-full bg-indigo-500 dark:bg-teal-500"
        />
      )}
      {'to' in props ? (
        <CloseButton
